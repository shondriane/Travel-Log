...
const Home = () => {
  ...
  const getActivityDetails = async () => {
    const response = await axios.get(`${BASE_URL}/activityDetails`)
          ...
          <div>
            ...
            <section className="search-results container-grid">
              {searchResults.map((searchResult) => (
                <Link
                  to={`/activityDetails/${searchResult._id}`}
        ...
        <section className="container-grid">
          {destination.map((country) => (
            <Link to={`/destination/${country._id}`} key={country._id}>
              <DestinationCard image={country.image} name={country.country} />
            </Link>
          ))}
        </section>
      </div>
